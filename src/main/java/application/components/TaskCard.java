...
public class TaskCard extends HBox {
    ...
    private void initUI(){
        ...
        Region priority = new Region();
        priority.setPrefHeight(40);
        priority.setPrefWidth(4);

        String pColor = "Transparent";
        if(task.getPriority()==3) pColor ="#47FF26";
        else if(task.getPriority()==1) pColor ="#FF5252";
        else if(task.getPriority()==2) pColor ="#FFA726";

        priority.setStyle("-fx-background-color: "+pColor+"; -fx-background-radius: 2");

        CheckBox checkBox = new CheckBox();
        checkBox.getStyleClass().add("task-checkbox");
        checkBox.setSelected(task.isCompleted());
        checkBox.setOnAction(event -> handleCompletedStatus());
        ...
        if(task.getDueDate()!=null){
            dateText.append("Due: ").append(task.getDueDate().format(dateTimeFmt));
        }

        if(task.isCompleted() &&task.getCompletedAt()!=null){
            dateText.append(" | Done: ").append(task.getCompletedAt().format(dateTimeFmt));
            dateLabel.setStyle("-fx-text-fill: #4CAF50;");
        } else if(!task.isCompleted() && task.getDueDate().isBefore(java.time.LocalDateTime.now())); {
        ...
        if(task.getLabels()!=null){
            for(Label label:task.getLabels()){
                ...
                try{
                    Color c = Color.web(label.getColorCode());
        ...
        this.getChildren().addAll(priority,checkBox, contentBox, btnEdit, btnDelete);
    }
